...
class ModernInvoiceApp:
    def __init__(self, root):
        ...
        try:
            ...
            FIREBASE_KEY_PATH = os.path.join(os.path.expanduser("~"), "billing-key_Invoice.json")
...
• CTRL+Z works for text fields
    ...
    def function_key_handler(self, event):
        ...
        key_map = {
            ...
            'F6': self.show_reports_dashboard,
    ...
    def show_modern_dashboard(self):
        ...
        quick_actions = [
            ...
            ("📊 View Reports", self.show_reports_dashboard, "F6", "Generate business reports"),
    ...
    def show_product_management(self):
        ...
        actions = [
            ...
            ("📊 Stock Report", self.show_stock_report, "View stock levels")
        ]
    ...
    def create_gui_compact(self, parent):
        ...
        header_label = tk.Label(parent, text="Estimate", font=("Arial", 16, "bold"), 
        ...
        # CGST and SGST Fields (Visible for South)
        self.cgst_label = tk.Label(gst_fields_frame, text="CGST %:", font=("Arial", 10),
                                bg=self.colors['light_bg'])
        self.cgst_label.grid(row=0, column=0, sticky=tk.W, padx=5)
        
        self.cgst = tk.DoubleVar(value=0.0)
        self.cgst_entry = tk.Entry(gst_fields_frame, textvariable=self.cgst, width=10, font=("Arial", 10))
        self.cgst_entry.grid(row=0, column=1, sticky=tk.W, padx=5)

        self.sgst_label = tk.Label(gst_fields_frame, text="SGST %:", font=("Arial", 10),
                                bg=self.colors['light_bg'])
        self.sgst_label.grid(row=0, column=2, sticky=tk.W, padx=5)
        
        self.sgst = tk.DoubleVar(value=0.0)
        self.sgst_entry = tk.Entry(gst_fields_frame, textvariable=self.sgst, width=10, font=("Arial", 10))
        self.sgst_entry.grid(row=0, column=3, sticky=tk.W, padx=5)

        # IGST Field (Visible for North)
        self.igst_label = tk.Label(gst_fields_frame, text="IGST %:", font=("Arial", 10),
                                bg=self.colors['light_bg'])
        self.igst_label.grid(row=0, column=4, sticky=tk.W, padx=5)
        
        self.igst = tk.DoubleVar(value=0.0)
        self.igst_entry = tk.Entry(gst_fields_frame, textvariable=self.igst, width=10, font=("Arial", 10))
    ...
    def generate_pdf(self):
        ...
        try:
            ...
            # Define the path to the Invoice_Bill directory in the user's Documents folder
            documents_dir = os.path.join(os.path.expanduser("~"), "Documents")
            ...
            # Create office-specific directory path with agent subfolder
            invoice_bill_dir = os.path.join(documents_dir, "InvoiceApp", "Invoice_Bill", office_folder, clean_agent_name)

            # Ensure the office-specific directory with agent subfolder exists
            if not os.path.exists(invoice_bill_dir):
                os.makedirs(invoice_bill_dir)
            ...
            relative_pdf_path = os.path.join("InvoiceApp", "Invoice_Bill", office_folder, clean_agent_name, f"{clean_customer_name}_{padded_bill_no}_{timestamp}.pdf")
    ...
    def get_pdf_path(self, relative_path):
        """
        Enhanced PDF path resolution with better error handling
        """
        try:
            documents_dir = os.path.join(os.path.expanduser("~"), "Documents")
            absolute_path = os.path.join(documents_dir, relative_path)
            ...
            self.show_status_message(f"❌ Error resolving PDF path: {e}", error=True)
            # Fallback: return the path as-is (might be absolute path from old data)
            return relative_path
    ...
    def update_gst_fields(self):
        ...
        try:
            if self.region.get() == "South":
                ...
                gst_percentage = float(self.gst_percentage.get())
                self.cgst.set(gst_percentage / 2)
                self.sgst.set(gst_percentage / 2)
                ...
                # Update IGST percentage
                self.igst.set(float(self.gst_percentage.get()))
    ...
    def calculate_total(self):
        ...
        try:
            ...
            try:
                ...
                if self.region.get() == "South":
                    # CGST and SGST
                    cgst_amount = (subtotal_with_packing * (self.cgst.get() or 0)) / 100
                    sgst_amount = (subtotal_with_packing * (self.sgst.get() or 0)) / 100
                    igst_amount = 0.0
                else:
                    # IGST
                    cgst_amount = 0.0
                    sgst_amount = 0.0
                    igst_amount = (subtotal_with_packing * (self.igst.get() or 0)) / 100
    ...
    def on_gst_percentage_change(self, *args):
        """Handle GST percentage change and update GST fields accordingly"""
        try:
            gst_percentage = float(self.gst_percentage.get())
            
            if self.region.get() == "South":
                # For South region, split GST equally between CGST and SGST
                cgst_sgst_percentage = gst_percentage / 2
                self.cgst.set(cgst_sgst_percentage)
                self.sgst.set(cgst_sgst_percentage)
                self.igst.set(0.0)
            else:
                # For North region, use full GST as IGST
                self.igst.set(gst_percentage)
                self.cgst.set(0.0)
                self.sgst.set(0.0)
                
            # Recalculate totals if there are items in the table
    ...
    def on_bill_selection_change(self, event):
        ...
        if selected:
            ...
            if bill_data:
                ...
                office = bill_data[7]  # ✅ ADDED: Get office from table
        ...
        if selected:
            ...
            if bill_data:
                ...
                self.selected_view_bill_info.config(
                    text=f"ℹ️  Selected: {bill_no} - {customer_name} | Amount: {net_amount} | Status: {status} | Office: {office} | {access_text}",
                    fg=status_color
                )
    ...
    def generate_agent_statement(self):
        ...
        # Save
        statements_dir = os.path.join(os.path.expanduser("~"), "Documents", "InvoiceApp", "Invoice_Bill", "STATEMENTS_FOLDER", "Agent_Statement_Only")
    ...
    def generate_customer_statement(self, event=None):
        ...
        try:
            ...
            documents_dir = os.path.join(os.path.expanduser("~"), "Documents")
            statements_dir = os.path.join(documents_dir, "InvoiceApp", "Invoice_Bill", "STATEMENTS_FOLDER", "Customer_All_Bills")
    ...
    def generate_agent_commission_only(self):
        ...
        try:
            ...
            statements_dir = os.path.join(
                os.path.expanduser("~"), "Documents", "InvoiceApp", "Invoice_Bill", "STATEMENTS_FOLDER", "Agent_Commission_Only"
            )
            os.makedirs(statements_dir, exist_ok=True)
    ...
    def generate_agent_statement_bills(self):
        ...
        try:
            ...
            statements_dir = os.path.join(
                os.path.expanduser("~"),
                "Documents", "InvoiceApp", "Invoice_Bill", "STATEMENTS_FOLDER", "Agent_All_Bills_Olny"
            )
            os.makedirs(statements_dir, exist_ok=True)
    ...
    def generate_agent_commission_statement(self):
        ...
        try:
            ...
            # Save commission PDF
            statements_dir = os.path.join(os.path.expanduser("~"), "Documents", "InvoiceApp", "Invoice_Bill", "STATEMENTS_FOLDER", "Agent_Commission_and_Bills")
            os.makedirs(statements_dir, exist_ok=True)
    ...
    def resolve_pdf_path_updated(self, pdf_file_path, bill_no):
        """
        RESOLVE PDF PATH - Checks in ALL office folders:
        - Documents/InvoiceApp/Invoice_Bill/AP
        - Documents/InvoiceApp/Invoice_Bill/AFI  
        - Documents/InvoiceApp/Invoice_Bill/AFF
        - Documents/InvoiceApp/Invoice_Bill/Invoice_Bills
        """
        ...
        documents_dir = os.path.join(os.path.expanduser("~"), "Documents")
        base_invoice_folder = os.path.join(documents_dir, "InvoiceApp", "Invoice_Bill")
        
        # Define ALL office folders to search
        office_folders = [
            "AP",
            "AFI", 
            "AFF",
            "Invoice_Bills"  # Keep the original folder as fallback
        ]
        
        # Extract filename from the path
        filename = os.path.basename(pdf_file_path)
        
        # Search in ALL office folders
        for office_folder in office_folders:
            office_path = os.path.join(base_invoice_folder, office_folder)
            
            # Try 1: Direct file path in the office folder
            direct_path = os.path.join(office_path, filename)
            if os.path.exists(direct_path):
                print(f"DEBUG: ✅ PDF found in {office_folder}: {direct_path}")
                return direct_path
            
            # Try 2: With agent subfolder in office folder
            ...
            if agent_name:
                ...
                if os.path.exists(agent_file_path):
                    print(f"DEBUG: ✅ PDF found in {office_folder}/{clean_agent_name}: {agent_file_path}")
                    return agent_file_path
        
        # Try 3: Search for any PDF with bill number in filename across ALL office folders
        search_patterns = [f"*{bill_no}*.pdf", f"*{bill_no}*.PDF"]
        
        for pattern in search_patterns:
            for office_folder in office_folders:
                office_path = os.path.join(base_invoice_folder, office_folder)
                try:
                    matches = glob.glob(os.path.join(office_path, "**", pattern), recursive=True)
                    if matches:
                        print(f"DEBUG: ✅ PDF found via search in {office_folder}: {matches[0]}")
                        return matches[0]
                except Exception as e:
                    continue
        
        print(f"DEBUG: ❌ PDF NOT FOUND for bill {bill_no} in any office folder")
        return None
    ...
    def show_reports_dashboard(self):
        """Show reports dashboard - TO BE IMPLEMENTED"""
        messagebox.showinfo("Coming Soon", "Reports Dashboard will be implemented next")
        
    def show_settings(self):
        """Show settings screen - TO BE IMPLEMENTED"""
        messagebox.showinfo("Coming Soon", "Settings screen will be implemented next")
